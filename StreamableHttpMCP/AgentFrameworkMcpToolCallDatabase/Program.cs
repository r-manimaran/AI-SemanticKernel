using AgentFrameworkMcpToolCallDatabase;
...
McpClient databaseMcpClient = await McpClient.CreateAsync(new HttpClientTransport(new HttpClientTransportOptions
{
    Endpoint = new Uri("http://localhost:5200/mcp/"),
    TransportMode = HttpTransportMode.StreamableHttp,
}));

IList<McpClientTool> toolsInDBMcp = await databaseMcpClient.ListToolsAsync();

AIAgent agent = client.GetChatClient(LLMConfig.DeploymentOrModelId)
    .AsAIAgent(
    instructions: "You are a Database expert in creating DB Queries. Analyze the user query and use appropriate tools from the MCP",
    tools: toolsInDBMcp.Cast<AITool>().ToList()
    ).AsBuilder()
